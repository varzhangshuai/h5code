$(function(){
    ...
    function changeImg(status,str) {
        ...
        if(status==0){
            //未提交
            $('.status-img').attr('src','http://img.daodaoclub.com/app/userauth/img/auth_1.png');
            $('.top-word').html('实名认证，请提交个人真实信息');
            $('.apply-auth').text('立即申请认证')
        }else if(status=='2'){
            //审核通过
            $('.status-img').attr('src','http://img.daodaoclub.com/app/userauth/img/auth_3.png');
            ...
            //审核不通过
            $('.status-img').attr('src','http://img.daodaoclub.com/app/userauth/img/auth_2.png?v=1.0')
